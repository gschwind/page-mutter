...
#define META_WINDOW_TILED(w)          ((w)->tile_mode == META_TILE_TILED)
...
#define META_WINDOW_ALLOWS_MOVE(w)     ((w)->has_move_func && !(w)->fullscreen && !META_WINDOW_TILED(w))
#define META_WINDOW_ALLOWS_RESIZE_EXCEPT_HINTS(w)   ((w)->has_resize_func && !META_WINDOW_MAXIMIZED (w) && !META_WINDOW_TILED_SIDE_BY_SIDE(w) && !(w)->fullscreen && !(w)->shaded && !META_WINDOW_TILED(w))
